...
EditableText.insert = function(Collection,doc) {
  ...
  if (self.customInsert && _.isString(self.customInsert)) {
    var callbackFunction = EditableText._callbacks[this.customInsert];
    if (callbackFunction && _.isFunction(callbackFunction)) {
      return callbackFunction.call(self, Collection, doc);
    }
  }
...
EditableText.update = function(Collection,doc,modifier) {
  var self = this;
  if (self.customUpdate && _.isString(self.customUpdate)) {
    var callbackFunction = EditableText._callbacks[this.customUpdate];
    if (callbackFunction && _.isFunction(callbackFunction)) {
      return callbackFunction.call(self, Collection, doc, modifier);
    }
  }
  if (EditableText.useMethods && this.useMethod !== false || this.useMethod) {
...
EditableText.remove = function(Collection,doc) {
  var self = this;
  if (self.customRemove && _.isString(self.customRemove)) {
    var callbackFunction = EditableText._callbacks[this.customRemove];
    if (callbackFunction && _.isFunction(callbackFunction)) {
      return callbackFunction.call(self, Collection, doc);
    }
  }
...
EditableText._makeUpdate = function(value,isEscape,evtType) {
  ...
  switch (type) {
    ...
    default :
      ...
      currentValue = _.isString(currentValue) && currentValue.replace(/<br \/>/g,"<br>") || currentValue;
      break;
  }
  var updatedValue = {};
  ...
  if (newValue !== currentValue || (!(newValue || (this.type === 'number' && newValue === 0 && keepZero)) && this.removeEmpty && !(this.acceptEmpty && evtType === 'focusout'))) {
    if (!(newValue || (this.type === 'number' && newValue === 0 && keepZero)) && this.removeEmpty && !(this.acceptEmpty && evtType === 'focusout')) {
      ...
      // Make the update
      if (newValue === '') {
        updatedValue[this.field] = '';
        EditableText.update.call(this,Mongo.Collection.get(this.collection),this.context,{$unset: updatedValue});
      }
      else {
...
Template.editable_text_widget.helpers({
  ...
  substitute : function(userCanNotEdit) {
    var value = EditableText._drillDown(this.context, this.field);
    if (value !== undefined && value !== '') {
      return '';
    }
    var substitute = !(this.value || value) && ((userCanNotEdit) ? (this.emptyText && Spacebars.SafeString(this.emptyText.toString())) : (this.substitute && Spacebars.SafeString(this.substitute.toString())));
	  return substitute;
  },
  ...
  content : function() {
    ...
    if (this.display && _.isString(this.display)) {
      var callbackFunction = EditableText._callbacks[this.display];
      if (callbackFunction && _.isFunction(callbackFunction)) {
        return callbackFunction.call(this.context, value);
      }
    }
    return val; 
  },
  ...
  safeFieldName: function(field) {
    return this.field.replace(/\./g,'');
  },
