...
EditableText._callback = function(callback, doc, originalValue) {
  ...
  if (originalValue) {
	...
	if (callbackRan && mutatedDocIsObject && (_.has(doc, '$set') || _.has(doc, '$unset') || _.has(doc, '$addToSet') || _.has(doc, '$push'))) {
	  return {modifier: doc};
	}
	else {
	  return {value: doc};  
      ...
      switch (action) {
        ...
        case 'update' :
          if (Meteor.isServer) {
            ...
            newValue = (modifier["$set"] && modifier["$set"][data.field]) || (modifier["$unset"] && modifier["$unset"][data.field]) || (modifier["$addToSet"] && modifier["$addToSet"][data.field]) || (modifier["$push"] && modifier["$push"][data.field]);
          }
		  ...
		  var callbackModifier = function (modifier, key, val) {
			...
			var modifierTypes = ["$set", "$unset", "$addToSet", "$push"];
